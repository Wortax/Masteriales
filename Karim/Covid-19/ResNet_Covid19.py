...
label_pass = "labels_Covid-19.csv"
...
class ResNetTabular(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_blocks=3, dropout=0.3):
        ...
        self.prediction = Prediction(hidden_size)
...
class TorchResNetModel(BaseEstimator, ClassifierMixin):
    def __init__(self, input_size, hidden_size=128, num_blocks=3, dropout=0.3, learning_rate=0.001, weight_decay=1e-4, epochs=10):
    ...
    def fit(self, X, y):
        ...
        self.model = ResNetTabular(self.input_size, self.hidden_size, self.num_blocks, self.dropout).to(device)
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            ...
            return (torch.sigmoid(outputs).cpu().numpy() >= 0.5).astype(int)
...
param_grid = {
    'hidden_size': [64, 128, 256],  # Taille des couches
    'num_blocks': [1, 2, 3, 4],    # Nombre de blocs ResNet
    'dropout': [0.2, 0.3, 0.4],    # Taux de dropout
    'learning_rate': [0.001, 0.01, 0.0001],
    'weight_decay': [1e-4, 1e-5, 1e-6],
    'epochs': [10, 20, 30]
}

model = TorchResNetModel(input_size=X_train.shape[1])
